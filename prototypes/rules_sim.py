...
可運行、無死鎖，並做蒙地卡羅初探平衡。發現的規則缺口寫入戰報。
...
# ── 帝國過度擴張：收益愈高的區域，衛冕投入折減愈大（攻方不受影響）──
...
1. **選卡時序矛盾**——DESIGN.md 將「收入階段」排喺「行動卡階段」之前，
   但制裁／油價戰的效果掛在收入階段：收入計算時其實已得知選卡結果選卡。
...
def main():
    ...
    report = ["# PROXY WAR v2 — 帝國過度擴張 A/B 戰報 R2\n",
              "> 問題：收益愈大的區域愈難守住（衛冕投入按區收入折減，攻方全額）能否修正 R1 發現的經濟流滾雪球？\n",
    ...
    for vname, vdesc, k in VARIANTS:
        ...
        report.append("")
    report.append("---")
    report.append("*同 1600 場/變體、同 seed 集、正反座對照；兩派 AI 未因新機制調整策略（刻意——驗證舊打法會否被懲罰）。*")
