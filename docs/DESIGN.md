...
- **勝負**：4 回合後戰略分高者勝；平手比國庫；對局中斷線見 §3.1a 斷線處理

### 3.1a 斷線與重連（2026-08-27 修訂）

| 情境 | 處理 |
|---|---|
| 對局中意外斷線（關頁/刷新/斷網） | 進入 **90 秒重連寬限期**：座位與身份保留，對手端顯示金色倒數提示 |
| 寬限期內同帳號 WS 重連 | 自動認回原座位、推送完整 state、廣播 `reconnected`；對局無損繼續 |
| 寬限期滿未歸 | 判負（「中斷線逾時，棄賽判負」），ELO 照入帳 |
| 主動棄賽（`leaveGame` 訊息／房內「離開」按鈕） | 不進寬限，即時判負 |

實作要點：`config.rooms.reconnectGraceMs` 參數化；寬限由伺服器 5 秒心跳檢查到期；
協議新增 `disconnectGrace` 視圖欄位、`reconnected` 與 `leaveGame` 訊息。
判負終局的 gameOver 廣播與統計入帶冪等保護（`overAnnounced`），與正常結算路徑互斥。
...
**規則**：收益愈大的戰區愈難守住。結算時，現任控制者的防守投入按以下公式折減，進攻方不受影響：
...
**設計意圖**：帝國過度擴張——佔領不等於統治。真實原型：游擊戰消耗佔領軍、阿富汗「帝國墳場」、冷戰代理人戰爭的佔領區治理成本。
...
### 5.1 模擬發現的規則缺口（TS 重寫前須補，來源：`prototypes/rules_sim.py` 戰報）
